...
TIMEOUT = 5
SIMULAR_ERROR = False  # 👈 Cambia a True para simular fallo en opendoor
...
async def handle_server_message(ws, message):
    ...
    if cmd == "opendoor":
        ...
        # Simulamos que el dispositivo abre la puerta correctamente o falla según la configuración
        response = {"ret": "opendoor", "result": not SIMULAR_ERROR}

        if SIMULAR_ERROR:
            response["reason"] = 1

        await asyncio.sleep(2)  # Simula un pequeño retraso del hardware
